...
(defun maybe-initiate-xp-printing (object fn stream &rest args)
  (if (xp-structure-p stream)
      (if sys::*circularity-counter*
          (if sys::*np-circularity-hash-table*
              (let ((marker (gethash object sys::*np-circularity-hash-table*)))
                   (if (eql 0 marker)
                       (progn (setf (gethash object sys::*np-circularity-hash-table*)
                                    (incf sys::*circularity-counter*))
                              (sys::print-label (gethash object sys::*np-circularity-hash-table*)
                                                (sys:out-synonym-of stream))
                              (apply fn stream args))
                       (if (and (not (not marker)) (not (eql t marker)))
                           (sys::print-reference (- marker) (sys:out-synonym-of stream))
                           (apply fn stream args))))
              (apply fn stream args))
          (if (and *print-circle* sys::*np-circularity-hash-table*)
              (let ((xnp1 (gethash object sys::*np-circularity-hash-table*)))
                   (apply fn stream args)
                   (when (listp object)
                         (if xnp1
                             (setf (gethash object sys::*np-circularity-hash-table*) 0)
                             (setf (gethash object sys::*np-circularity-hash-table*) t))))
              (apply fn stream args)))
      (let ((*abbreviation-happened* nil)
        ...
        (if (and *print-circle* (null sys::*circularity-hash-table*))
            (let ((sys::*circularity-hash-table* (make-hash-table :test 'eq))
                  (sys::*np-circularity-hash-table* (make-hash-table :test 'eq)))
